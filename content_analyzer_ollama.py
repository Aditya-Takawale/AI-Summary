...
IMPORTANT: Return ONLY the JSON object, nothing else."""
    ...
    def _validate_result(self, result: Dict):
        ...
        for i, question in enumerate(result["quiz"]):
            ...
            # Check if correct_answer is in options (exact match)
            ...
            if correct_answer not in options:
                ...
                if normalized_correct in normalized_options:
                    # Fix the correct_answer to match the exact option
                    idx = normalized_options.index(normalized_correct)
                    question["correct_answer"] = options[idx]
                    logger.warning(f"Quiz question {i+1}: Fixed correct_answer case mismatch")
                else:
